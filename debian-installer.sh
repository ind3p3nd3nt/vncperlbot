...
case $choice in
 Y) sudo apt update && sudo apt install python perl pkg-perl-tools libssl-dev masscan -y && sudo cpan -fi Net::SSLeay IO::Socket::SSL Time::HiRes Mojolicious Mojo::IRC;;  
esac
...
if read -p "Masscan RATE-LIMIT/Recommended: 25000" mrate; then
 sed -i~ -e "s|--rate 2048|--rate "$mrate"|g" perlircssl.pl
fi
